...
impl<T: Trait> Module<T> {
    ...
    pub fn create_vote(
        ...
        if vote_type == VoteType::Binary {
            ensure!(outcomes.len() == 2, "Invalid binary outcomes")
        }
        if vote_type == VoteType::MultiOption {
            ensure!(outcomes.len() > 2, "Invalid multi option outcomes")
        }
        if vote_type == VoteType::RankedChoice {
            ensure!(outcomes.len() > 2, "Invalid ranked choice outcomes")
        }

        let id = Self::vote_record_count() + 1;
        <VoteRecords<T>>::insert(
            id,
            VoteRecord {
                id: id,
                commitments: vec![],
    ...
    pub fn is_ranked_choice_vote_valid(
        mut vote: Vec<VoteOutcome>,
        mut outcomes: Vec<VoteOutcome>,
    ) -> bool {
